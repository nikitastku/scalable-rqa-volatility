...
Output:
  - results/benchmark_scalable_rqa.txt  (detailed timing + accuracy results)
  - figures/7_1_rqa_samp_accuracy.png   (measure accuracy vs sample size M)
  - figures/7_2_rqa_timing_scaling.png   (wall-clock time vs N)
  - figures/7_3_rqa_pipeline_comparison.png (bar chart: method timing)

Usage:
  python scripts/evaluate/benchmark_scalable_rqa.py
  python scripts/evaluate/benchmark_scalable_rqa.py --max_tickers 50   # quick test
  python scripts/evaluate/benchmark_scalable_rqa.py --full              # all 503 stocks
"""
...
def rqa_worp(X: np.ndarray, eps: float, lmin: int = 2, vmin: int = 2) -> dict[str, float]:
    """
    RQA without recurrence plot construction.
    """
    n = X.shape[0]
    eps2 = eps * eps
    total_recurrent = 0
...
def rqa_samp(
    ...
    est_total_recurrent = int(rr * n * (n - 1))

    diag_ge = diag_arr[diag_arr >= lmin]
    det = float(diag_ge.sum() / max(est_total_recurrent, 1)) if est_total_recurrent > 0 else 0.0
    lmax = float(diag_ge.max()) if diag_ge.size else 0.0

    vert_ge = vert_arr[vert_arr >= vmin]
    lam = float(vert_ge.sum() / max(est_total_recurrent, 1)) if est_total_recurrent > 0 else 0.0
    tt = float(vert_ge.mean()) if vert_ge.size else 0.0
...
def rqa_worp_vectorised(X: np.ndarray, eps: float, lmin: int = 2, vmin: int = 2) -> dict[str, float]:
    """
    Vectorised RQA without recurrence plot construction.
    """
    n = X.shape[0]
    total_recurrent = 0
    eps2 = eps * eps
...
def benchmark_windowed_pipeline(
    ...
    """
    Benchmark B: compare windowed approaches on one stock.
    """
    ...
    t0 = time.time()
    X_current = rqa_features_rolling(df_stock, rqa_cols, rqa_cfg, prefix="rqa", eps_fixed=eps_fixed)
    t_current = time.time() - t0

    t0 = time.time()
    X_samp4 = rqa_samp_features_rolling(
        df_stock, rqa_cols, rqa_cfg, M_factor=4.0, prefix="rqa_s4", eps_fixed=eps_fixed
    )
    t_samp4 = time.time() - t0

    t0 = time.time()
    X_samp1 = rqa_samp_features_rolling(
        df_stock, rqa_cols, rqa_cfg, M_factor=1.0, prefix="rqa_s1", eps_fixed=eps_fixed
    )
    t_samp1 = time.time() - t0

    logger.info(f"    Windowed exact:      {t_current:.2f}s")
    logger.info(f"    Windowed Samp M=4N:  {t_samp4:.2f}s  (speedup: {t_current/max(t_samp4,1e-6):.1f}x)")
    logger.info(f"    Windowed Samp M=1N:  {t_samp1:.2f}s  (speedup: {t_current/max(t_samp1,1e-6):.1f}x)")
...
def plot_timing_scaling(single_results, fig_name):
    ...
    if t_samp4:
        ax.plot(N_vals[:len(t_samp4)], t_samp4, "^-", color=COLORS["green"], linewidth=2, label="RQA_Samp M=4N")
    if t_samp1:
        ax.plot(N_vals[:len(t_samp1)], t_samp1, "v-", color=COLORS["purple"], linewidth=2, label="RQA_Samp M=N")
    if t_samp02:
        ax.plot(N_vals[:len(t_samp02)], t_samp02, "d-", color=COLORS["orange"], linewidth=2, label="RQA_Samp M=0.2N")
...
def plot_pipeline_comparison(windowed_results, pipeline_total, fig_name):
    ...
    times = [windowed_results["t_current"], windowed_results["t_samp4"], windowed_results["t_samp1"]]
    ...
    for bar, t in zip(bars, times):
        ax.text(t + 0.02, bar.get_y() + bar.get_height() / 2, f"{t:.2f}s", va="center", fontsize=10)
    ...
    for bar, t, label in zip(bars, proj_times, proj_labels):
        ax.text(t + 5, bar.get_y() + bar.get_height() / 2, label, va="center", fontsize=10)
...
def main():
    ...
    eps = estimate_epsilon_from_train(df_stock[list(rqa_cols)].to_numpy(dtype=float), rqa_cfg)
    ...
    if n_bench > 1:
        ...
        for i, ticker in enumerate(subset_tickers):
            ...
            if (i + 1) % max(1, n_bench // 5) == 0:
                logger.info(f"  Current exact: {i+1}/{n_bench} ({time.time()-t0:.1f}s)")
        t_multi_current = time.time() - t0

        t0 = time.time()
        for i, ticker in enumerate(subset_tickers):
            df_t = train[train["ticker"] == ticker].reset_index(drop=True)
            rqa_samp_features_rolling(df_t, rqa_cols, rqa_cfg, M_factor=4.0, prefix="rqa_s", eps_fixed=eps)
            if (i + 1) % max(1, n_bench // 5) == 0:
                logger.info(f"  Samp M=4N: {i+1}/{n_bench} ({time.time()-t0:.1f}s)")
        t_multi_samp = time.time() - t0

        logger.info(
            f"  {n_bench} stocks — Exact: {t_multi_current:.1f}s, "
        ...
        if n_bench < len(tickers):
            ...
            logger.info(
                f"  Projected 503 stocks — Exact: {proj_exact:.0f}s ({proj_exact/60:.1f}min), "
            ...
            logger.info(
                f"  FULL 503 stocks — Exact: {proj_exact:.0f}s ({proj_exact/60:.1f}min), "
                f"Samp: {proj_samp:.0f}s ({proj_samp/60:.1f}min)"
            )
    ...
    for r in single_results:
        ...
        lines.append(
            f"  {r['N']:>6} {t_std:>10.4f}s {t_worp:>10.4f}s "
